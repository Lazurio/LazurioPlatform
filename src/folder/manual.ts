...
const workingHere = document("Working here", generated, [
  ...
  "Create worktrees, commit and push to the PR branch continuously, and open pull requests. Work never stays only local: from the first push it is visible as an open pull request. The product concept “Draft” is the editable work itself, not GitHub's draft state. GitHub's draft-PR state may be used while work is in progress only where the owning repository's `AGENTS.md` allows it; some repositories require a review-ready pull request from the first push, and the repository's own rule wins. Once the work is finished and verified you mark the pull request ready for review yourself. Ready is not Publication; it only says “ready to be checked”. Finished work never stays a Draft, and a remote branch without a pull request is not a handoff.",
  "",
  ...
  "- On a handover: request review from the eligible Principal they chose, assign them the PR and hand over the responsibility with an @mention. A reviewer request means “check”; the assignee owns the next work and edits the PR branch directly. Assignment creates no rights and replaces no Publication instruction.",
